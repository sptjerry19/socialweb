<template>
  <div v-if="isNamed == false">
    <input
      class="rounded-sm border-solid border-black border-spacing-24 border"
      type="text"
      v-model="currentUser"
    ...
    <div class="flex-1 overflow-y-auto p-4 scrollContainer">
      <div class="flex flex-col space-y-2">
        ...
        <div
          ...
          :key="message.id"
          class="flex"
          :class="{ 'justify-end': message.user == currentUser }"
        >
          <div
            v-if="message.user !== currentUser"
            class="text-black p-2 max-w-xs"
          >
            {{ message.user }}:
          </div>
          <div
...
export default {
  ...
  methods: {
    ...
    submitName() {
      ...
      localStorage.setItem("name", this.currentUser);
      this.currentUser !== "" ? (this.isNamed = true) : false;
      console.log(this.isNamed);
